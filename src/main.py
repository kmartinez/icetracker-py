...
if __name__ == "__main__":
      if ADMIN_IO.value:
            supervisor.set_next_code_file("./adminmode.py")
      ...
      if RTC_DEVICE.alarm1_status:
         """ALARM in future forces the device to reset immediatley upon waking up, need solution around this..."""
         """Devices can now wake up at certain time windows throughout the day"""
         # if RTC_DEVICE.alarm_is_in_future():
         #    logger.critical("Abnormal reset detected!!! Shutting device down")
         #    PSU.shutdown()
         # else:
            # RTC_DEVICE.alarm1 = (localtime(mktime(RTC_DEVICE.alarm1[0])+TIME_INTERVAL), "monthly")
            # RTC_DEVICE.alarm1 = (localtime(mktime(RTC_DEVICE.datetime)+30), "monthly")
         (YY, MM, DD, hh, mm, ss, wday, yday, dst) = RTC_DEVICE.datetime
         nextwake_MM = get_next_alarm_min(mm)
         RTC_DEVICE.alarm1 = (struct_time([YY,MM,DD,hh,nextwake_MM,0,wday,yday,dst]), "monthly")
         print(RTC_DEVICE.alarm1)
